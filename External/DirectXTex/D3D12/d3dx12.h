...
struct CD3DX12_PIPELINE_STATE_STREAM2
{
    ...
    CD3DX12_PIPELINE_STATE_STREAM2(const D3D12_GRAPHICS_PIPELINE_STATE_DESC& Properties) noexcept
        : Flags(Properties.Flags)
        , NodeMask(Properties.NodeMask)
        , pRootSignature(Properties.pRootSignature)
        , InputLayout(Properties.InputLayout)
        , IBStripCutValue(Properties.IBStripCutValue)
        , PrimitiveTopologyType(Properties.PrimitiveTopologyType)
        , VS(Properties.VS)
        , GS(Properties.GS)
        , StreamOutput(Properties.StreamOutput)
        , HS(Properties.HS)
        , DS(Properties.DS)
        , PS(Properties.PS)
        , BlendState(CD3DX12_BLEND_DESC(Properties.BlendState))
        , DepthStencilState(CD3DX12_DEPTH_STENCIL_DESC1(Properties.DepthStencilState))
        , DSVFormat(Properties.DSVFormat)
        , RasterizerState(CD3DX12_RASTERIZER_DESC(Properties.RasterizerState))
        , RTVFormats(CD3DX12_RT_FORMAT_ARRAY(Properties.RTVFormats, Properties.NumRenderTargets))
        , SampleDesc(Properties.SampleDesc)
        , SampleMask(Properties.SampleMask)
        , CachedPSO(Properties.CachedPSO)
        , ViewInstancingDesc(CD3DX12_VIEW_INSTANCING_DESC(CD3DX12_DEFAULT()))
    {}
    CD3DX12_PIPELINE_STATE_STREAM2(const D3DX12_MESH_SHADER_PIPELINE_STATE_DESC& Properties) noexcept
        : Flags(Properties.Flags)
        , NodeMask(Properties.NodeMask)
        , pRootSignature(Properties.pRootSignature)
        , PrimitiveTopologyType(Properties.PrimitiveTopologyType)
        ...
        , ViewInstancingDesc(CD3DX12_VIEW_INSTANCING_DESC(CD3DX12_DEFAULT()))
    {}
    CD3DX12_PIPELINE_STATE_STREAM2(const D3D12_COMPUTE_PIPELINE_STATE_DESC& Properties) noexcept
        : Flags(Properties.Flags)
        , NodeMask(Properties.NodeMask)
        , pRootSignature(Properties.pRootSignature)
        , CS(CD3DX12_SHADER_BYTECODE(Properties.CS))
        , CachedPSO(Properties.CachedPSO)
    {
...
struct CD3DX12_PIPELINE_STATE_STREAM1
{
    ...
    {}
#if defined(NTDDI_WIN10_VB) && (NTDDI_VERSION >= NTDDI_WIN10_VB)
    CD3DX12_PIPELINE_STATE_STREAM1(const D3DX12_MESH_SHADER_PIPELINE_STATE_DESC& Properties) noexcept
        : Flags(Properties.Flags)
        , NodeMask(Properties.NodeMask)
        , pRootSignature(Properties.pRootSignature)
        , PrimitiveTopologyType(Properties.PrimitiveTopologyType)
...
struct CD3DX12_PIPELINE_MESH_STATE_STREAM
{
    ...
    CD3DX12_PIPELINE_MESH_STATE_STREAM(const D3DX12_MESH_SHADER_PIPELINE_STATE_DESC& Properties) noexcept
        : Flags(Properties.Flags)
        , NodeMask(Properties.NodeMask)
        , pRootSignature(Properties.pRootSignature)
        , PS(Properties.PS)
