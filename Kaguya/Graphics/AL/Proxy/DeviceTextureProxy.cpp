...
void DeviceTextureProxy::SetWidth(UINT64 Width)
{
	m_Width = Width;
}

void DeviceTextureProxy::SetHeight(UINT Height)
{
	m_Height = Height;
}

void DeviceTextureProxy::SetDepthOrArraySize(UINT16 DepthOrArraySize)
{
	m_DepthOrArraySize = DepthOrArraySize;
}

void DeviceTextureProxy::SetMipLevels(UINT16 MipLevels)
{
	m_MipLevels = MipLevels;
}

void DeviceTextureProxy::SetClearValue(D3D12_CLEAR_VALUE ClearValue)
{
	m_ClearValue = std::make_optional<D3D12_CLEAR_VALUE>(ClearValue);
}

void DeviceTextureProxy::Link()
{
	assert
	(
