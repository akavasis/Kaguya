...
CommandAllocator::CommandAllocator(const Device* pDevice, D3D12_COMMAND_LIST_TYPE Type)
{
	ThrowCOMIfFailed(pDevice->GetD3DDevice()->CreateCommandAllocator(Type, IID_PPV_ARGS(&m_CmdAllocator)));
}

void CommandAllocator::Reset()
{
	ThrowCOMIfFailed(m_CmdAllocator->Reset());
}
